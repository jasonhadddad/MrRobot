...
void wallFollow(void){
    ...
    if(adcRAW < (setADCdist - 80)) {
        ...
        ADJUSTED_SPEED_R = CURRENT_SPEED_R;  
    ...
    else if((adcRAW > setADCdist && leftWall) || (adcRAW < setADCdist && !leftWall)){   

            ADC_ADJUST = adcRAW - setADCdist;
            ADJUSTED_SPEED_L = CURRENT_SPEED_L*(4/100)*ADC_ADJUST + ((7/3)*(CURRENT_SPEED_L))/(ADC_ADJUST+10);
            ADJUSTED_SPEED_R = CURRENT_SPEED_R;        
    } 
            ...
            ADJUSTED_SPEED_R = CURRENT_SPEED_R*(4/100)*ADC_ADJUST + (2*(CURRENT_SPEED_R))/(ADC_ADJUST+10);
            ADJUSTED_SPEED_L = CURRENT_SPEED_L;       
    }
...
//    ser_putch(4);				//Song 4
 ...
 //   DriveDirect(250,250);
  ...
  //      if ((getSensorData(13,1)) == 1){  //If virtual wall or home-base force field sensor triggered. Maybe we could test in turn 90 as well.
            lcdWriteToDigitBCD((getSensorData(17,1)));     
            if((getSensorData(17,1)) < 240){                                   //If not the force-field. Used to be <240
                IR_Wall = 1;   
                distance = 1001;
                RB3 = 0;
            }
