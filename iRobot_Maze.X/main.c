...
void wallFollow(void){
    ...
    unsigned int setADCdist = 200;
                   leftWall = 1;          
    ADCMain();  
   

            if(adcRAW < (setADCdist - 90)) {   //&& leftWall == 1
        ...
        ADJUSTED_SPEED_R = CURRENT_SPEED_R;   
            ...
            ADJUSTED_SPEED_L = (14/10)*CURRENT_SPEED_L + (4/10)*ADC_ADJUST + ((CURRENT_SPEED_L))/(ADC_ADJUST+10);
            ADJUSTED_SPEED_R = CURRENT_SPEED_R;        
            ...
            ADJUSTED_SPEED_R = (14/10)*CURRENT_SPEED_R + (4/10)*ADC_ADJUST + ((CURRENT_SPEED_R))/(ADC_ADJUST+10);
            ADJUSTED_SPEED_L = CURRENT_SPEED_L;    
...
if(getSensorData(13,1) == 1){
             
    
     if ((omni_IR == 246) || (omni_IR == 250)){    
         homeBase = 1;
     }
            switch(omni_IR){
                case 250: homeBase = 1;
